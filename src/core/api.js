...
async function searchTenders(args = {}) {
  ...
  const result = await ekap.searchTenders({
    ...
    // Varsayılan: sadece başlık + nitelik/tür/miktar. Teknik şartname/sözleşme
    // taslağı gibi uzun belgelerde tesadüfen geçen kelimeler gürültü yapar.
    searchInIkn: args.searchInIkn ?? false,
    searchInTitle: args.searchInTitle ?? true,
    searchInAnnouncement: args.searchInAnnouncement ?? false,
    searchInTechSpec: args.searchInTechSpec ?? false,
    searchInAdminSpec: args.searchInAdminSpec ?? false,
    searchInSimilarWork: args.searchInSimilarWork ?? false,
    searchInLocation: args.searchInLocation ?? false,
    searchInNatureQuantity: args.searchInNatureQuantity ?? true,
    searchInTenderInfo: args.searchInTenderInfo ?? false,
    searchInContractDraft: args.searchInContractDraft ?? false,
    searchInBidForm: args.searchInBidForm ?? false,
    skip: args.skip ?? 0,
    limit,
  });
